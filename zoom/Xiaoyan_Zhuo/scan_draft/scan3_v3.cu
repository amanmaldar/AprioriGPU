...
__global__ void scatter_incr(int *d_array, int *d_sums2)
{
  const size_t bx = 2 * blockDim.x * blockIdx.x;
  const size_t tx = threadIdx.x;
  const unsigned int u = d_sums2[blockIdx.x];
  d_array[bx + 2*tx]   += u;
  d_array[bx + 2*tx+1] += u;
}
...
int main(){
    ...
    // cudaMallocHost((void **) &g_out2, sizeof(int)*len);
    ...
    scatter_incr<<<nblocks, blksize/2>>>(d_out, d_sums2);
    cudaMemcpy(g_out, d_out, sizeof(int)*len, cudaMemcpyDeviceToHost);
    cudaThreadSynchronize();
