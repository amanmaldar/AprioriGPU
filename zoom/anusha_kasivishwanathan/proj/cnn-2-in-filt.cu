...
__global__ void conv2D_cuda(double *x, double *w, double *b, double *o, int *o_int, int ksize, int str, int pad, int pad_size, int relu, int pool, int in_filt, int out_filt)
{
...
class cnn
{
	...
	double *conv2D(double *x, double *w, double *b, int ksize, int stride, int padding, int relu, int pool, vector <int> in_shape, vector<int> out_shape)
	{
		...
		conv2D_cuda<<<num_blocks, num_threads, sizeof(double)*datasize>>>(x_d, w_d, b_d, o_d, o_int, ksize, stride, padding, pad_size, relu, pool, in_shape[3], out_shape[3]);
