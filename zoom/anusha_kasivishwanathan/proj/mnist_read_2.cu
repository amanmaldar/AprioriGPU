...
__global__ void par_signed_dist(double *data, int num_im, int data_size, double *eucl_dist, int eucl_dist_size, double *signed_dist, int option, int *signed_dist_int )
{
    ...
    __shared__ double tmp_dist[784];
    int tot_blks = num_im;
    while(bid<(tot_blks))
    {
        int this_im     = bid / data_size;
        int cnt         = (bid% data_size) * blockDim.x+tid;
        ...
        // if(bid==0)
        //     printf("%d %d %d %d %d %d\n", bid, tid, this_im, cnt, from_pix, to_pix);
        ...
        if(option==0)
        {
            ...
            for(int i=blockDim.x/2;i>0;i=i/2)
            {
                if(tid < i)
                {    
                    if(tmp_dist[tid] > tmp_dist[tid+i])
                        tmp_dist[tid] = tmp_dist[tid+i];
                    if(i%2 != 0 && i != 1)
                        i++;
                    // if(bid==0)
                    // printf("thread %d, i=%d, @i=%f, @tid+i=%f\n", tid, i, tmp_dist[tid], tmp_dist[tid+i]);
                }
                __syncthreads();
            }


            if(tid==0 && tmp_dist[tid]<signed_dist[dist_idx])
            {
                if(bid==0)
                    printf("block ZERO %f %f\n\n\n", tmp_dist[tid], signed_dist[dist_idx]);
                signed_dist[dist_idx] = tmp_dist[tid];
            }
        }
        else
        {
            atomicMin(&(signed_dist_int[dist_idx]),(int)(tmp_dist[tid]*100000));
            __syncthreads(); 
            if(tid==0)
            { 
                signed_dist[dist_idx] = (double)signed_dist_int[dist_idx]/100000;
            }
...
class dataset
{
    ...
    {      
        ...
        {
            signed_dist[i]=10000000;
            signed_dist_int[i]=10000000;
        }
        ...
        cout<<"Time to find signed dist:"<<time_to_mul_us<<" microseconds, about "<<time_to_mul_ms<<" milliseconds\n";
    ...
    double *compute_eucl_dist(int eucl_dist_size, int num_cols)
    {
       ...
       cout<<"\n\n\n\n\n\n\n\nCompute eucl dist"; 
        ...
        auto time_to_mul_us = chrono::duration_cast<chrono::microseconds>(end_time_mul-start_time_mul).count();
        auto time_to_mul_ms = chrono::duration_cast<chrono::milliseconds>(end_time_mul-start_time_mul).count();
        cout<<"\n\n\n\nTime to find eucl dist:"<<time_to_mul_us<<" microseconds, about "<<time_to_mul_ms<<" milliseconds\n";
        return eucl_dist;
    }
...
int main()
{
    ...
    // tr_im.disp_eucl_dist(eucl_dist, num_rows, num_cols);

    // for(int i=0;i<784;i++)
    // {
    //     if(i%28==0)
    //         printf("\n");
    //     printf("%d ",(int)eucl_dist[i]);

    // }
    // printf("\n");

    ofstream my_file("eucl_pix_0.dat", ios::binary);
        for(int k=0;k<784;k++)
        {
            if(k%28==0)
                my_file << "\n";
        ...
        my_file << "\n\n";
        my_file.close();
    double *signed_dist1= tr_im.compute_signed_dist(tr_data, num_im, num_rows*num_cols, eucl_dist, eucl_dist_size,1);
