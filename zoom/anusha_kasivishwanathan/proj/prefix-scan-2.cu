...
int main(int argc, char **argv)
{
	...
	cudaMalloc((void**)&blk_sums_2_d, sizeof(int)*(n/(num_threads*num_threads*4)+1));
	// cudaMalloc((void**)&blk_sums_3_d, sizeof(int)*1);
	cudaMalloc((void**)&blk_sums_3_d, sizeof(int)*(n/(num_threads*num_threads*num_threads*8)+1));
	cudaMalloc((void**)&blk_sums_4_d, sizeof(int)*1);

	cudaMalloc((void**)&blk_sums_1_out_d, sizeof(int)*n/(num_threads*2));
	cudaMalloc((void**)&blk_sums_2_out_d, sizeof(int)*(n/(num_threads*num_threads*4)+1));
	cudaMalloc((void**)&blk_sums_3_out_d, sizeof(int)*(n/(num_threads*num_threads*num_threads*8)+1));
	...
	// int final_sums[32];
	int *final_sums = (int*)malloc(sizeof(int)*n);
	...
	cout<<"first few a values:\n";
	cudaMemcpy(final_sums, a_d, sizeof(int)*n, cudaMemcpyDeviceToHost);
	for(int i=0;i<32;i++)
		cout<<final_sums[i]<<' ';
	cout<<"\n\n";
	cout<<"first few final sums:\n";
	cudaMemcpy(final_sums, o_d, sizeof(int)*n, cudaMemcpyDeviceToHost);
	for(int i=0;i<32;i++)
		cout<<final_sums[i]<<' ';

	cout<<"\n\n";

	auto time_us = chrono::duration_cast<chrono::microseconds>(end_time-start_time).count();	
	auto time_ms = chrono::duration_cast<chrono::milliseconds>(end_time-start_time).count();	
	
	cout<<"\nTime to conv:"<<time_us<<" microseconds, about "<<time_ms<<" milliseconds\n";
